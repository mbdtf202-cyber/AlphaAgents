...
This checklist lets a buyer sponsor inspect whether the sandbox package is internally traceable. It does not prove real payment or real customer demand.
...
7. Open `16-cli-api-ui-snapshots.json` and confirm UI, CLI, and API `orderDto` objects match.
...
Pass condition: all referenced evidence ids exist in `09-evidence-index.csv`, all sampled rows have `qaStatus=passed`, and no sampled evidence row claims `validated` commercial proof.
