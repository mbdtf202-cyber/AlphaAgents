...
export function getAgentListings() {
  return listings.map((listing) => enrichListing(listing));
}
...
export function getFeaturedListings() {
  return getAgentListings().filter((listing) => listing.featured);
}
...
export function getListingsByAgentId(agentId) {
  return getAgentListings().filter((listing) => listing.agentId === agentId);
}
...
function enrichListing(listing) {
  const agent = listing.supplyType === "agent_app"
    ? agentApps.find((agentApp) => agentApp.id === listing.agentId)
    : agents.find((entry) => entry.id === listing.agentId);
  const categoryTags = listing.categoryIds.flatMap((categoryId) => getCategoryById(categoryId)?.tags ?? []);
  const tags = Array.from(new Set([...(listing.tags ?? []), ...(agent?.tags ?? []), ...categoryTags])).sort((a, b) =>
    a.localeCompare(b)
  );

  return {
    ...listing,
    agentName: agent?.name ?? listing.agentId,
    tags
  };
}
