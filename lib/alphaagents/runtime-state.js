...
const DEFAULT_STATE_FILE = path.resolve("output", "alphaagents-runtime-state.json");
...
function buildInitialState() {
  ...
  return {
    ...
    evidenceRecords: structuredClone(evidenceRecords).map((record) => ({
      version: 1,
      ...record
    })),
    appInstalls: [],
...
export function ensureRuntimeState(stateFile) {
  ...
  if (!fs.existsSync(resolved)) {
    writeJsonAtomic(resolved, buildInitialState());
  }
