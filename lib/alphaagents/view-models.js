...
export function filterCatalogListings(listings, filters = {}) {
  ...
  return listings.filter((listing) => {
    ...
    return (
      ...
      (!normalized.tag || listing.tags?.includes(normalized.tag)) &&
...
export function getWorkbenchModel(options = {}) {
  ...
  const appRuns = runtimeSnapshot.appUsageRuns.slice(-3).map((run) => ({
    ...
    acceptanceStatus: run.acceptanceStatus ?? "missing",
    reputationStatus: run.reputationStatus ?? "missing"
  }));
...
export function getRiskFinanceModel(options = {}) {
  ...
  const auditEvents = ["PermissionRequested", "PermissionApproved", "PermissionDenied", "PermissionRevoked"];
  return {
    shell: getShellModel(),
    financeRules: contract.financeRules,
    security: contract.security,
    highRiskPermissionPreview: {
      deniedTools,
      previewFields: ["grantId", "orderId", "agentId", "toolAllowlist", "expiresAt", "approvalReason", "revocationReason"],
      authorizationOwners: ["operator", "buyer acceptance owner", "finance contact for fund-related tools"],
      auditEvents,
      revokeCommand: "alphaagents permission revoke --grant-id <grantId> --revocation-reason <reason> --json",
      blockedUntil: "explicit authorization, buyer-safe preview, audit event, and revoke path are visible"
    },
    riskActionChecklist: [
      {
        gate: "explicit_authorization",
        label: "Explicit authorization",
        command: "alphaagents permission approve --grant-id <grantId> --tool-allowlist read_public_url,write_generated_artifact --json",
        evidence: "PermissionApproved event with operator actor, tool allowlist, expiry, and evidence refs"
      },
      {
        gate: "action_preview",
        label: "Action preview",
        command: "alphaagents permission approve --dry-run --grant-id <grantId> --json",
        evidence: "Preview shows grantId, orderId, agentId, toolAllowlist, expiresAt, approvalReason, and denied-tool diff"
      },
      {
        gate: "audit_event",
        label: "Audit event",
        command: "alphaagents events show --aggregate <grantId> --json",
        evidence: auditEvents.join(", ")
      },
      {
        gate: "revoke_path",
        label: "Revoke path",
        command: "alphaagents permission revoke --grant-id <grantId> --revocation-reason buyer_cancelled_scope --json",
        evidence: "PermissionRevoked event blocks future runtime execution with that grant"
      }
    ],
    categoryUnitEconomics: getCategoryUnitEconomicsRows(),
...
export function getAgentAppDetailModel(slug, options = {}) {
  ...
  const ownerAgent = getAgentPassportById(app.ownerAgentId);
  ...
  return {
    ...
    runtimeUsageRuns,
    latestInstall,
    latestUsageRun: runtimeUsageRuns.at(-1) ?? null,
...
function getAgentPassportById(agentId) {
  return getAgentPassports().find((agent) => agent.id === agentId) ?? null;
}
